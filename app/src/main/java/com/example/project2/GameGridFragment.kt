...
class GameGridFragment : Fragment(R.layout.grid) {
    ...
    private fun squareChecker(){
        ...
        for(i in 0..squares.size - 1){
            var neighbors = 0
            //tracks rows, columns
            val rowCount = squares.size / COLUMN_COUNT
            val row = i/rowCount // if 2 then 2nd row
            val col = i % COLUMN_COUNT
            val right_col = (i + 1) % COLUMN_COUNT
            val left_col = (i + COLUMN_COUNT-1) % COLUMN_COUNT
            var top_row = (row + rowCount-1) % rowCount
            var bot_row = (row + 1) % rowCount
